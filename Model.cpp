...
{
    //新たなModel型のインスタンスのメモリ確保
    Model* model = new Model();
	//デスクリプタヒープの生成
	model->InitializeDescriptorHeap();
	//読み込み
	model->LoadFromInternal(modelname);
	//バッファ生成
	model->CreateBuffers();
...
void Model::LoadFromInternal(const std::string&modelname)
{
	...
	vector<XMFLOAT2>texcoords;//テクスチャ3UV
	//1行ずつ読み込む
	string line;
	while (getline(file, line)) {
		...
		if (key == "f")
		{
			...
			{
				//頂点インデックス1個分の文字数をストリームに変換して解析しやすくする
				std::istringstream index_stream(index_string);
				unsigned short indexPosition, indexNormal, indexTexcoord;
				index_stream >> indexPosition;
				index_stream.seekg(1, ios_base::cur);//スラッシュとばす
...
void Model::CreateBuffers()
{
	...
	sizeIB = static_cast<UINT>(sizeof(indices));
